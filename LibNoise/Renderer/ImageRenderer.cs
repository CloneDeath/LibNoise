...
// right edge
						...
						var nl = _noiseMap.GetValue(x + xLeftOffset, y);
						var nr = _noiseMap.GetValue(x + xRightOffset, y);
						var nd = _noiseMap.GetValue(x, y + yDownOffset);
		...
		{
			var sourceRed = sourceColor.Red / 255.0f;
			var sourceGreen = sourceColor.Green / 255.0f;
			var sourceBlue = sourceColor.Blue / 255.0f;
			var sourceAlpha = sourceColor.Alpha / 255.0f;

			var backgroundRed = backgroundColor.Red / 255.0f;
			var backgroundGreen = backgroundColor.Green / 255.0f;
			var backgroundBlue = backgroundColor.Blue / 255.0f;

			// First, blend the source color to the background color using the alpha
			// of the source color.
			var red = Libnoise.Lerp(backgroundRed, sourceRed, sourceAlpha);
			var green = Libnoise.Lerp(backgroundGreen, sourceGreen, sourceAlpha);
			var blue = Libnoise.Lerp(backgroundBlue, sourceBlue, sourceAlpha);

			if (_lightEnabled)
			{
				// Now calculate the light color.
				var lightRed = lightValue * _lightColor.Red / 255.0f;
				var lightGreen = lightValue * _lightColor.Green / 255.0f;
				var lightBlue = lightValue * _lightColor.Blue / 255.0f;
	    ...
	    private float CalcLightIntensity(float center, float left, float right, float down, float up)
		{
			...
			var io = iMax * Libnoise.Sqrt2 * _sinElevation / 2.0f;
			var ix = (iMax - io) * _lightContrast * Libnoise.Sqrt2 * _cosElevation * _cosAzimuth;
			var iy = (iMax - io) * _lightContrast * Libnoise.Sqrt2 * _cosElevation * _sinAzimuth;
			var intensity = ix * (left - right) + iy * (down - up) + io;
